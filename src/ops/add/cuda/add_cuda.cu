#include "add_cuda.hpp"

#include "../../../core/llaisys_core.hpp"
#include "../../../device/nvidia/nvidia_dtype.cuh"
#include "../../../utils.hpp"

#include <cuda_runtime_api.h>

#include <cstdint>

namespace llaisys::device::nvidia {
void cuda_check(cudaError_t rc, const char *what, const char *file, int line);
} // namespace llaisys::device::nvidia

#define LLAISYS_CUDA_CHECK(call) \
    ::llaisys::device::nvidia::cuda_check((call), #call, __FILE__, __LINE__)

namespace llaisys::ops::cuda {

namespace {

template <typename T>
__global__ void add_kernel(T *c, const T *a, const T *b, std::uint64_t numel) {
    const std::uint64_t i = static_cast<std::uint64_t>(blockIdx.x) * blockDim.x
                            + static_cast<std::uint64_t>(threadIdx.x);
    if (i >= numel) {
        return;
    }
    const float av = llaisys::device::nvidia::dtype::to_float<T>(a[i]);
    const float bv = llaisys::device::nvidia::dtype::to_float<T>(b[i]);
    c[i] = llaisys::device::nvidia::dtype::from_float<T>(av + bv);
}

template <typename T>
void launch_add(tensor_t c, tensor_t a, tensor_t b, std::uint64_t numel) {
    constexpr int kBlock = 256;
    const int grid = static_cast<int>((numel + static_cast<std::uint64_t>(kBlock) - 1)
                                      / static_cast<std::uint64_t>(kBlock));
    auto stream = reinterpret_cast<cudaStream_t>(llaisys::core::context().runtime().stream());
    add_kernel<T><<<grid, kBlock, 0, stream>>>(
        reinterpret_cast<T *>(c->data()),
        reinterpret_cast<const T *>(a->data()),
        reinterpret_cast<const T *>(b->data()),
        numel);
    LLAISYS_CUDA_CHECK(cudaGetLastError());
}

} // namespace

void add(tensor_t c, tensor_t a, tensor_t b) {
    const std::uint64_t numel = static_cast<std::uint64_t>(c->numel());
    if (numel == 0) {
        return;
    }

    switch (c->dtype()) {
    case LLAISYS_DTYPE_F32:
        launch_add<float>(c, a, b, numel);
        return;
    case LLAISYS_DTYPE_F16:
        launch_add<llaisys::fp16_t>(c, a, b, numel);
        return;
    case LLAISYS_DTYPE_BF16:
        launch_add<llaisys::bf16_t>(c, a, b, numel);
        return;
    default:
        EXCEPTION_UNSUPPORTED_DATATYPE(c->dtype());
    }
}

} // namespace llaisys::ops::cuda
